...
- [网络编程](#网络编程)
- [1. 客户端-服务器编程模型](#1-客户端-服务器编程模型)
- [2. 网络](#2-网络)
    - [2.1 网络的物理结构](#21-网络的物理结构)
    - [2.2 主机之间通网络的数据传输](#22-主机之间通网络的数据传输)
- [3. 全球IP因特网](#3-全球ip因特网)
    - [3.1 IP地址](#31-ip地址)
    - [3.2 域名解析](#32-域名解析)
    - [3.3 网络程序连接](#33-网络程序连接)
- [4. 套接字API](#4-套接字api)
    - [4.1 客户端的行为](#41-客户端的行为)
    - [4.2 服务器的行为](#42-服务器的行为)
    - [4.3 `ip:port`号与`sockaddr`之间的转换](#43-ipport号与sockaddr之间的转换)
- [5. Web服务器](#5-web服务器)
    - [5.1 Web基础](#51-web基础)
    - [5.2 Web内容](#52-web内容)
    - [5.3 HTTP协议](#53-http协议)
        - [请求](#请求)
        - [响应](#响应)
    - [5.4 服务动态内容](#54-服务动态内容)
        - [客户端如何将程序参数传递给服务器](#客户端如何将程序参数传递给服务器)
        - [服务端是如何将参数传递给子进程的](#服务端是如何将参数传递给子进程的)
        - [子进程将它的输出发送到哪里](#子进程将它的输出发送到哪里)
- [6. TINY Web 服务器](#6-tiny-web-服务器)
- [7. 参考资料](#7-参考资料)
...
> 参考阅读：[从输入URL到页面展示到底发生了什么？](https://www.cnblogs.com/xianyulaodi/p/6547807.html)
...
HTTP协议规定客户端的请求消息，必须符合下面的格式：

![](./images/ch11_10.png)

主要有三部分组成：请求行、零个或多个请求报头，再跟随一个空的文本行来终止报头列表，最后是主体部分，用于传递参数等。
- HTTP支持许多不同的方法，包括：GET、POST、HEAD、PUT、DELETE等。
- 请求报头为服务器提供了额外的信息，例如浏览器的商标名，或者浏览器理解的MIMI类型。
- 报头中的Host报头，看上去是有点重复，实际是用于告知代理缓存服务器，原始服务器的地址。
...
结构上与请求报文类似，一个响应行，后面跟随着零个或更多的响应报头，再跟随一个终止报头的空行，最后是响应主体。

我们需要注意的一个是响应行中的状态码， 不同的状态码对应的不同的含义。另一个需要注意的是响应报头中的`Content-Length`，它明确了响应主体部分的字节数。
...
Web服务器能够向客户端提供动态的内容，使得Web的功能更加强大，增加了用户与Web页面的互动性。那服务器要实现动态请求，需要解决什么问题呢？

### 客户端如何将程序参数传递给服务器

客户端通过GET与POST两种类型的请求都可以传递参数到服务端，一般来说GET用来传递简单的参数，POST用来传递复杂的参数。

GET的参数是写在请求行上的，与请求的动态程序URI之间通过`？`进行分割，不同的参数之间参数`&`分割 。如`GET /cgi-bin/adder?15000&213 HTTP/1.1`。

### 服务端是如何将参数传递给子进程的

服务器端在接收到动态内容请求后，它会调用`fork`来创建一个子进程，并调用`execve`在子进程的上下文中执行`/cgi-bin/adder`程序。服务器将请求相关的信息都写到子进程的环境变量中，子进程可以访问不同的环境变量来获取这些请求的参数。

### 子进程将它的输出发送到哪里

在子进程加载并运行CGI程序之前，它使用Linux dup2函数将标准输出重定向到和客户端相关的已连接的描述符。因为任何子进程写到标准输出的东西都会直接到达客户端。
...
源码：[tiny_web.c](tiny_web.c)

# 7. 参考资料

- [Guide to Network Programming](http://beej.us/guide/bgnet/html/single/bgnet.html)
- [Crow is C++ microframework for web. (inspired by Python Flask)](https://github.com/ipkn/crow)
- [Industrial-grade RPC framework used throughout Baidu](https://github.com/brpc/brpc)